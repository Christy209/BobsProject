...
public class IssuseCheckbookForCreatedSavingsAccount{
	...
	public IssuseCheckbookForCreatedSavingsAccount(WebDriver driver) {
	        this.driver = driver;
	  ...
	  public Map<String, String> execute(RowData inputData, RowData id,RowData vr, String sheetName, int i, String excelPath) throws Exception {
		  ...
		  try {
	           ...
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("acctNum"), 8), id.getByHeader("Created_AccountID"));
	           
	           String chqAckValue = inputData.getByIndex(3); 
	           if ("Y".equalsIgnoreCase(chqAckValue)) {

	               WebElement yesRadio = wait.until(
	                   ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='chqAckObtFlg' and @value='Y']")));
	               yesRadio.click();

	           } else if ("N".equalsIgnoreCase(chqAckValue)) {

	               WebElement noRadio = wait.until(
	                   ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='chqAckObtFlg' and @value='N']")));
	               noRadio.click();
	           }
	           
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("chqType"), 8), inputData.getByIndex(4));
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("beginChqAlphaFrom"), 8), inputData.getByIndex(5));
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("beginChqSrlNumFrom"), 8), inputData.getByIndex(6));
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("beginChqAlphaTo"), 8), inputData.getByIndex(7));
	           WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("beginChqSrlNumTo"), 8), inputData.getByIndex(8));
              ...
              // 	popupHandled = clickAcceptRobust(mainWindowHandle, 3); 
               ...
               try {
            	    ...
            	    if (fullText.contains("-")) {
            	        accountNumber = fullText.substring(fullText.lastIndexOf("-") + 1).trim();
            	    }
