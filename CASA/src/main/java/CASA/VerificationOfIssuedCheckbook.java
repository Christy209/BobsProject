...
public class VerificationOfIssuedCheckbook {
...
public VerificationOfIssuedCheckbook(WebDriver driver) {
        this.driver = driver;
  ...
  public Map<String, String> execute(RowData inputData, RowData id,RowData vr, String sheetName, int i, String excelPath) throws Exception {
      ...
      //**************************************Verification & Validation Methods**************************************************//
	   private static Map<String, String> getApplicationData(RowData inputData, WebDriver driver, WebDriverWait wait) throws Exception {
	    ...
	    Map<String, String> confirmationDetails = extractCheckbookDetailsFromConfirmation();
	    appData.putAll(confirmationDetails);
	    return appData;
	   }
	   ...
	   try {
         ...
         try {
        	    ...
        	    WebElement label = wait.until(
        	        ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='compField']")));

        	    String fullText = label.getText().trim();
        	    System.out.println("Full Message: " + fullText);
        	    String accountNumber = fullText.replaceAll("[^0-9]", "");
        	    ...
        	    if (!accountNumber.isEmpty()) {
        	        ...
        	        result.put("errorMsg", "Account ID not found in label");
        	        return result;
        	    }

        	} catch (Exception e) {
        	    e.printStackTrace();
        	    result.put("errorMsg", "Exception while extracting Account ID");
        	    return result;
        	}
...
private static Map<String, String> extractCheckbookDetailsFromConfirmation() {
    Map<String, String> details = new HashMap<>();
    try {
        WebElement label = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='compField']")));
        String fullText = label.getText().trim();
        // Example parsing logic: adjust as per actual message format
        // Assume message contains: "Checkbook Issued: Type=SB, From=60251, To=60300, AlphaFrom=70L, AlphaTo=70L"
        if (fullText.contains("Checkbook Issued")) {
        ...
        details.put("confirmationMessage", fullText);
    } catch (Exception e) {
        details.put("errorMsg", "Failed to extract checkbook details from confirmation.");
    }
    return details;
}
}
