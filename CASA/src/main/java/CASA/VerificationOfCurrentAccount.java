...
public class VerificationOfCurrentAccount{
	...
	public VerificationOfCurrentAccount(WebDriver driver) {
		   this.driver = driver;
	        this.wait = new WebDriverWait(driver, Duration.ofSeconds(10)); 
	    }
	  
	  public Map<String, String> execute(RowData rowData,RowData id,RowData vr, String sheetName, int i, String excelPath) throws Exception {

		   String mainWindowHandle = driver.getWindowHandle();
		Map<String, String> resultMap = new HashMap<>();
		String errorMsg = "";
		
		String menu = rowData.getByIndex(1);
		String funcode = rowData.getByIndex(2);
	        ...
	        try {
	            ...
	            System.out.println("⚠️ Could not switch to loginFrame");
	        }

	        WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("menuSelect"),5), menu);
	        ...
	        try {
	            ...
	            wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("FINW")));
			     ...
			     logWindowHandlesAndTitles();
	                ...
	                System.out.println("Using verifyCancel dropdown for HOAACVLA");
	                WebElement dropdown = wait.until(
	                        ExpectedConditions.visibilityOfElementLocated(By.id("verifyCancel")));
	                WindowHandle.selectByVisibleText(dropdown, funcode);
	                WindowHandle.setValueWithJS(driver, waitForVisibility(By.id("tempForacid"),5), id.getByHeader("Created_AccountID"));
	      ...
	      //----------------------------------------Verify--------------------------------------------------------------//
	        if("V - Verify".equalsIgnoreCase(rowData.getByIndex(2))) {
                Map<String, String> appData = getApplicationData(vr, driver, wait);
                try {
                    Validation.validateData(rowData.getHeaderMap(), appData);

                } catch (AssertionError ae) {
                    Thread.sleep(500);
                   appData = getApplicationData(vr, driver, wait);
                    Validation.validateData(rowData.getHeaderMap(), appData); // if fails again it will throw

                }
                clickSubmitAndHandlePopup(mainWindowHandle,excelPath,sheetName,i,resultMap,funcode);
                   return resultMap;
            }
			return resultMap;
	  }
...
//********************************General details tab**************************************************************************//
    ...
    String freqOption = vr.getByIndex(4);
...
try {
         ...
         appData.put("minIntPcntCr", ExcelUtils.getTextOrValue(driver, wait, "minIntPcntCr", "id"));
         appData.put("maxIntPcntCr", ExcelUtils.getTextOrValue(driver, wait, "maxIntPcntCr", "id"));
         appData.put("custPrefIntCr", ExcelUtils.getTextOrValue(driver, wait, "custPrefIntCr", "id"));
         appData.put("acctPrefIntCr", ExcelUtils.getTextOrValue(driver, wait, "acctPrefIntCr", "id"));
         appData.put("chanPrefIntCr", ExcelUtils.getTextOrValue(driver, wait, "chanPrefIntCr", "id"));
         appData.put("intCrAcctFlg", ExcelUtils.getTextOrValue(driver, wait, "intCrAcctFlg", "id"));
         appData.put("nextIntCrCalcDt_ui", ExcelUtils.getTextOrValue(driver, wait, "nextIntCrCalcDt_ui", "id"));
  }catch (Exception e) {
		 System.out.println("General details tab error : " + e.getMessage());
	 }
...
//*************************Miscodes tab******************************************************//
 ...
 //-----------------------------------------------logWindowHandlesAndTitles-------------------------------------------------------//
   private static void logWindowHandlesAndTitles() {
 	     try {
   ...
   private Map<String, String> clickSubmitAndHandlePopup(String mainWindowHandle,String excelPath,String sheetName,int rowIndex,Map<String, String> result,String menu) {
	...
	//*************************************************clickAcceptRobust******************************************************//
		...
		private static boolean clickAcceptRobust(String mainWindowHandle, int timeoutSeconds) {
