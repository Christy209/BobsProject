...
import CASA.InquireTransactionMaintainence;
...
public class TC36_InquireTheModifiedTransactionAsCrossCCYReceiptImplicit {
    public WebDriver driver;
	private WebDriverWait wait;
	
    private static final String EXCEL_PATH = System.getProperty("user.dir") + "/Resource/CASA.xlsx";
    private static final String SHEET_NAME = "TC36";
    ...
    public void runTest() throws Exception {
        ...
        String logFile = TestResultLogger
                .createLogFile("TC36_InquireTheModifiedTransactionAsCrossCCYReceiptImplicit");
        ...
        while (attempt <= maxRetry && !testPassed) {
            ...
            try {
                ...
                RowData id = ExcelUtils.getRowAsRowData("TC35", 1);
                RowData post = ExcelUtils.getRowAsRowData(SHEET_NAME, 1);

                login.First();
                WindowHandle.validateAlreadyLoggedIn(driver, logFile);

                InquireTransactionMaintainence tran =
                        new InquireTransactionMaintainence(driver);

                // ---------------- POST ----------------//
                Map<String, String> verifyResult =tran.executeWithResultMap(post, id,"TC36", 1, EXCEL_PATH);
